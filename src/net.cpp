...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Phantom is probably already running."), addrBind.ToString());
        else
...
static void run_tor() {
    ...
    if (fs::exists(torrc_defaults_file) && fs::is_regular_file(torrc_defaults_file))
    {
        ...
        {
            int check_rev = 0;
            fscanf(file, "# torrc-defaults for Phantom rev. %d\n", &check_rev);
    ...
    if (must_override || !fs::exists(torrc_defaults_file) || !fs::is_regular_file(torrc_defaults_file))
    {
        ...
        if (file)
        {
            fprintf(file, "# torrc-defaults for Phantom rev. %d\n", TORRC_DEFAULTS_REV);
            fprintf(file, "#\n");
            fprintf(file, "# DO NOT EDIT THIS FILE\n");
            fprintf(file, "#\n");
            fprintf(file, "# This file is distributed with Phantom within the tor folder and SHOULD NOT be modified\n");
            fprintf(file, "# (it may be overwritten during the next Phantom update).\n");
            fprintf(file, "# To customize your Tor configuration, shut down Phantom and edit the torrc file.\n");
            fprintf(file, "#\n");
