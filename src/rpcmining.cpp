...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    uint64_t nExpectedTime = staking ? (GetTargetSpacing(nBestHeight, GetAdjustedTime()) * nNetworkWeight / nWeight) : 0;
    ...
    obj.push_back(Pair("netstakeweightrecent", (uint64_t)GetPoSKernelPSRecent()));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Phantom is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Phantom is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Phantom is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Phantom is downloading blocks...");
